...
I wanted to build something closer to what FP&A teams build that would demonstrate operational and driver based forecasting, rather than a pure Wall Street-style valuation. DexCom is a medtech/consumables business (continuous glucose monitors) with:
...
Built as a 9 tab Excel workbook, fully linked end to end. Every output flows from the assumptions tab so there are no hardcoded numbers downstream.
...
These are the calls that most shaped the output, in order of impact:
...
At $45.49, this model's output sits meaningfully below where DexCom actually trades (~$89.75) -- I chose not to adjust assumptions to close that gap artificially. A few reasons the gap exists:
